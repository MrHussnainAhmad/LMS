...
export function StaffRequestsTabContent({
  active,
  campuses,
}: {
  active: boolean;
  ...
  useEffect(() => {
    ...
    api.get<{ requests: RequestRow[] }>("/api/institution/staff-profile-requests")
      .then((data) => {
        if (!ignore) setRequests(data.requests);
      })
      .catch(() => {
        if (!ignore) setError("Could not load staff requests. Please try again.");
      })
      .finally(() => {
  ...
  return <StaffRequestsClient requests={requests} campuses={campuses} />;
}
