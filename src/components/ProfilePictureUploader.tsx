...
export function ProfilePictureUploader({
  ...
  const [previewUrl, setPreviewUrl] = useState(isImageUrl(currentPictureUrl) ? currentPictureUrl : "");
  ...
  const uploadPicture = async (file: File) => {
    ...
    try {
      ...
      toast({ title: "Could not upload picture", description: errorMessage(error), variant: "destructive" });
    } finally {
  ...
  return (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
      <div className="flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-full border border-border bg-stone-50 shadow-sm">
        {previewUrl ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={previewUrl} alt={`${name} profile picture`} className="h-full w-full object-cover" />
        ) : (
          <span className="text-2xl font-bold text-brand-800">{name.substring(0, 2).toUpperCase()}</span>
        )}
      </div>
      <div className="space-y-3">
        <div>
          <p className="text-sm font-semibold text-brand-950">Profile Picture</p>
          <p className="text-sm text-stone-500">Upload a square PNG/JPG picture for your profile.</p>
        </div>
        <input
          ref={inputRef}
          type="file"
